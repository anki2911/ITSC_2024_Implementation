...
class Server:
    ...
    def attack_prevent(self,msg,key):
        h = hmac.new(str(key).encode(),str(msg).encode(),hashlib.sha512)
    ...
    def estimate_location(self):
        ...
        #Find the road ids where the vehicle may belong to based on the min and max lat and long
        ...
        if len(eligible_road) > 0:
            ...
            while (u < len(eligible_road)):
                if (float(min_d[u]) < float(d_min)):  
                    if ((prev_road_id == -1) and (cur_road_id == -1)):
                        #print("FirstEntry")
                        d_min = min_d[u]
                        index_min = u
                        Road_Cur = str(eligible_road[u]) 
                        #The first Road ID is initialized as 0
                        self.Vehicle_Data[index] = (id,cur_pos_x,cur_pos_y,cur_angle,cur_speed,cur_time,prev_road_id,str(Road_Cur))          
                    #On the same road
                    elif (str(cur_road_id) != -1) and (str(cur_road_id) == str(eligible_road[u])):
                        d_min = min_d[u]
                        index_min = u
                        Road_Cur = str(eligible_road[u])  
                        #print("Found")
                    #From one road to another road
                    elif ((str(cur_road_id)+"_"+str(eligible_road[u])) in self.Edge_Relation.keys()) and (((self.Edge_Relation[str(cur_road_id)+"_"+str(eligible_road[u])] == 1) or (self.Edge_Relation[str(eligible_road[u])+"_"+str(cur_road_id)] == 1))):
                        d_min = min_d[u]
                        index_min = u
                        Road_Cur = str(eligible_road[u]) 
                        #print(Road_Cur)
            ...
            #From a road to another road
                ...
                if ((self.Edge_Relation[str(cur_road_id)+"_"+str(Road_Cur)] == 1) or (self.Edge_Relation[str(Road_Cur)+"_"+str(cur_road_id)] == 1)):                       
                    if (lc1) < (len1 - lc1):
                        ...
                        r1 = len1 - lc1
                    print(str(lc1) + " " + str(r1))
                        
                    if (lc2) < (len2 - lc2):
                        r2 = lc2
                    else:
                        r2 = len2 - lc2
                    print(str(lc2) + " " + str(r2))

                    #Apply Log Martingle Error calculation over here
                    if r1 + r2 <= a:
                        flag_authentic = True
                        #print("No Error")
                        return (abs(r1 + r2))    
                        ...
                        return (math.sqrt((float(prev_pos_x) - float(cur_pos_x))*(float(prev_pos_x) - float(cur_pos_x)) + (float(prev_pos_y) - float(cur_pos_y))*(float(prev_pos_y) - float(cur_pos_y))))
                        #print("Error")
                        #return (r1 + r2)   

                #elif (math.sqrt((float(prev_pos_x) - float(cur_pos_x))*(float(prev_pos_x) - float(cur_pos_x)) + (float(prev_pos_y) - float(cur_pos_y))*(float(prev_pos_y) - float(cur_pos_y)))) <= a:
        ...
        #From a road to a junction
            ...
            for i in range(0,len(self.Edge_Junc)):
                if (str(self.Edge_Junc[i][0]) == str(cur_road_id)) and (found == 0):
                    ...
                    if (abs(float(cur_pos_x) - float(x1)) < abs(float(cur_pos_x) - float(x2))) and (abs(float(cur_pos_y) - float(y1)) < abs(float(cur_pos_y) - float(y2))):
                        print((abs(float(cur_pos_x) - float(x1))*abs(float(cur_pos_x) - float(x1))) + (abs(float(cur_pos_y) - float(y1))*abs(float(cur_pos_y) - float(y1))))
                        print((abs(float(prev_pos_x) - float(x1))*abs(float(prev_pos_x) - float(x1))) + (abs(float(prev_pos_y) - float(y1))*abs(float(prev_pos_y) - float(y1))))
                        print((abs(float(cur_pos_x) - float(x2))*abs(float(cur_pos_x) - float(x2))) + (abs(float(cur_pos_y) - float(y2))*abs(float(cur_pos_y) - float(y2))))
                        print((abs(float(prev_pos_x) - float(x2))*abs(float(prev_pos_x) - float(x2))) + (abs(float(prev_pos_y) - float(y2))*abs(float(prev_pos_y) - float(y2))))
                        cur_junc = end_1
                    else:
                        print((abs(float(cur_pos_x) - float(x1))*abs(float(cur_pos_x) - float(x1))) + (abs(float(cur_pos_y) - float(y1))*abs(float(cur_pos_y) - float(y1))))
                        print((abs(float(prev_pos_x) - float(x1))*abs(float(prev_pos_x) - float(x1))) + (abs(float(prev_pos_y) - float(y1))*abs(float(prev_pos_y) - float(y1))))
            ...
            if (found == 1):
                if (math.sqrt((float(prev_pos_x) - float(cur_pos_x))*(float(prev_pos_x) - float(cur_pos_x)) + (float(prev_pos_y) - float(cur_pos_y))*(float(prev_pos_y) - float(cur_pos_y)))) <= a:
                    flag_authentic = True
                    #print("No Error")
                    return (math.sqrt((float(prev_pos_x) - float(cur_pos_x))*(float(prev_pos_x) - float(cur_pos_x)) + (float(prev_pos_y) - float(cur_pos_y))*(float(prev_pos_y) - float(cur_pos_y))))
                else:
